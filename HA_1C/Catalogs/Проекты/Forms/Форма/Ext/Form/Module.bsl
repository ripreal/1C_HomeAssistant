...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.Сумма.Шрифт = Новый Шрифт(ШрифтыСтиля.ОбычныйШрифтТекста,,, Истина);
	Элементы.Сумма.РастягиватьПоГоризонтали = Истина;
	Элементы.Сумма.Вид = ВидПоляФормы.ПолеНадписи;
	Элементы.Сумма.Формат = "ЧЦ=7; ЧРГ=' '; ЧН='0 rub'; ЧГ=3,0; ЧФ='Ч rub'";
...
Процедура ИзменитьДату(Команда)
	...
	ФормыОбщиеДействияКлиент.НачатьВводДаты(ЭтотОбъект, Элементы.Дата, Объект.Дата);
...
Процедура ИзменитьОбъект(Команда)
	...
	Оповещение = Новый ОписаниеОповещения("ЗавершитьИзменениеОбъекта", ЭтотОбъект);
	ОткрытьФорму("Справочник.Объекты.ФормаВыбора",, Элементы.ОбъектОбъект,,,, Оповещение);
...
Процедура УправлениеФормой()
	...
	Элементы.ОбъектОбъект.Видимость = ЗначениеЗаполнено(Объект.Объект);
...
&НаКлиенте
Процедура ЗавершитьИзменениеОбъекта(ВыбранноеЗначение, ДополнительныеПараметры) Экспорт
	
	Если ВыбранноеЗначение = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Объект.Объект = ВыбранноеЗначение;
	УправлениеФормой();
...
Процедура СдвинутьВверх(Команда)
	...
	СдвинутьСтрокуВверх(Элементы.Детали.ТекущиеДанные.ПолучитьИдентификатор());
...
Процедура СдвинутьВниз(Команда)
	...
	СдвинутьСтрокуВниз(Элементы.Детали.ТекущиеДанные.ПолучитьИдентификатор());
КонецПроцедуры

Процедура СдвинутьСтрокуВверх(ИдСтроки)
	
	СтрокаДеталей = Объект.Детали.НайтиПоИдентификатору(ИдСтроки);
	ИндексСтроки = Объект.Детали.Индекс(СтрокаДеталей);
	Если ИндексСтроки > 0 Тогда
		Объект.Детали.Сдвинуть(ИндексСтроки - 1, 1);
	КонецЕсли;
	
КонецПроцедуры

Процедура СдвинутьСтрокуВниз(ИдСтроки)
	
	СтрокаДеталей = Объект.Детали.НайтиПоИдентификатору(ИдСтроки);
	ИндексСтроки = Объект.Детали.Индекс(СтрокаДеталей);
	Если ИндексСтроки < Объект.Детали.Количество() - 1 Тогда
		Объект.Детали.Сдвинуть(ИндексСтроки + 1, -1);
	КонецЕсли;
