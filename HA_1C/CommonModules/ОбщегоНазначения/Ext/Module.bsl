...
// Возвращает инициализированное начальное значение переданного значения
//
// Параметры:
//  Прототип - Любой - значение, для которого нужно получить начальное значение
// 
// Возвращаемое значение:
//  Любой - начальное значение прототипа
//
Функция НезаполненноеЗначение(Прототип) Экспорт 
	Если ТипЗнч(Прототип) = Тип("Строка") Тогда
		Возврат "";
	ИначеЕсли ТипЗнч(Прототип) = Тип("Число") Тогда
		Возврат 0; 
	ИначеЕсли ТипЗнч(Прототип) = Тип("Дата") Тогда
		Возврат Дата(1, 1, 1);
	ИначеЕсли ТипЗнч(Прототип) = Тип("УникальныйИдентификатор") Тогда
		Возврат  Новый УникальныйИдентификатор("00000000-0000-0000-0000-000000000000");
	ИначеЕсли ТипЗнч(Прототип) = Тип("ПеречислениеСсылка.СтатусыВыполненияТО") Тогда
		Возврат Перечисления.СтатусыВыполненияТО.Пройдено; 
	Иначе
...
//  ЛюбаяСсылка - 
//
Функция НайтиСоздатьПредопределенныйЭлемент(ПолноеИмяПредопределенного, ЭтоГруппа = Ложь) Экспорт
	...
	Если Не ЗначениеЗаполнено(ПредопрЭл) Тогда
		...
		Если ЭтоГруппа Тогда
			ПредопрЭл = МенеджерОб.СоздатьГруппу();
		Иначе
		...
		Наименование = "";
		Для ИндексБуквы = 1 По СтрДлина(ИмяЭлемента) Цикл
			БукваИмени = Сред(ИмяЭлемента, ИндексБуквы, 1);
			Если ИндексБуквы > 1 И БукваИмени = ВРег(БукваИмени) Тогда
				Наименование = Наименование + " " + НРег(БукваИмени);
			Иначе
				Наименование = Наименование + БукваИмени;
			КонецЕсли;
		КонецЦикла;
		
		ПредопрЭл.Наименование = Наименование;
