...
Процедура ПробегПриИзменении(Элемент)
	Если ИзмерительТО = ИзмерительТОПробег() Тогда
		Объект.ТекущийНомерТО = ТекущийНомерТО(Объект.Автомобиль, Объект.Дата, Объект.Пробег);
		ЗагрузитьТаблицуСПланомТО();
	КонецЕсли;
КонецПроцедуры
...
Процедура Инициализировать()
	...
	ИзмерительТО = ИзмерительТОАвто(Объект.Автомобиль);
...
Процедура ДобавитьСтрокуПланаТОНаФорму(СтрокаТаблицы, Счетчик)
	...
	Для каждого ВидТО ИЗ ВидыТО Цикл
		...
		// План ТО
		ОписанияТипа = Новый ОписаниеТипов;
		ОписанияТипа.Типы().Добавить(Тип("Дата"));
		ОписанияТипа.Типы().Добавить(Тип("Число"));
		ЭлементРеквизита = ФормыОбщиеДействияСервер.ДобавитьРеквизитСоСвязаннымЭлементомНаФорму(
			ЭтотОбъект, 
		...
		ЭлементРеквизита.Формат = ?(ИзмерительТО = Справочники.Автомобили.ИзмерительТО_ВРЕМЯ() Или СтрокаТаблицы[ВидТО + "Внеплановое"], "ДФ=dd/MM/yy; ДЛФ=D; ДП=-", "ЧЦ=6; ЧДЦ=0; ЧРГ=' '");
		ЭлементРеквизита.ТолькоПросмотр = Истина;
...
Процедура ЗагрузитьТаблицуСПланомТО()
	...
	СкрытьЭлементыТЧВыпРабот();
	
	Если Не ЗначениеЗаполнено(Объект.Автомобиль) Или Не ЗначениеЗаполнено(Объект.Дата) Тогда
		Объект.ВыполненныеРаботы.Очистить();
	Иначе
		Объект.ВыполненныеРаботы.Загрузить(ТаблицаПланТО(Объект.Автомобиль, ИзмерительТО, Объект.Дата, Объект.Пробег));
	КонецЕсли;
...
Процедура Подключаемый_СсылкаНаДатуТОНажатие(Элемент, СтандартнаяОбработка)
	...
	Если ИзмерительТО = ИзмерительТОВремя() Тогда
		ДефолтнаяДата = ?(ЗначениеЗаполнено(РеквизитДаты), РеквизитДаты, Макс(Объект.Дата, РеквизитДатыПлан));
	ИначеЕсли ИзмерительТО = ИзмерительТОПробег() Тогда
		ДефолтнаяДата = ?(ЗначениеЗаполнено(РеквизитДаты), РеквизитДаты, Объект.Дата);
	Иначе
...
Процедура Подключаемый_СуммаТОПриИзменении(Элемент, СтандартнаяОбработка)
	...
	СтатусТО = СтрокаТЧ[УзнатьВидТОПоИмениЭлемента(Элемент.Имя) + "Статус"];
...
Процедура ЗавершитьДобавлениеРаботы(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	ДатаТО = НачалоИнтервалаТО(ИмяТекущееТО(), Объект.Дата, Истина);
...
Процедура ОбновитьТумблерДиапазонаТО()
	...
	РеквизитыАвто = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.Автомобиль, "ДатаПриобретения,МежсервисныйИнтервал");
	...
	Для каждого ВидТО Из ВидыТО Цикл
		...
		Если ИзмерительТО = Справочники.Автомобили.ИзмерительТО_ВРЕМЯ() Тогда
			ДатаПоИнтервалу =  Документы.РегистрацииТО.ДатаТОПоИнтервалу(РеквизитыАвто.ДатаПриобретения, ВидТО, Объект.Дата);
		ИначеЕсли ИзмерительТО = Справочники.Автомобили.ИзмерительТО_ПРОБЕГ() Тогда
			ДатаПоИнтервалу =  Документы.РегистрацииТО.ПробегТОПоИнтервалу(ВидТО, Объект.ТекущийНомерТО,
	...
	Для каждого СтрокаРеглРаботы Из Объект.ВыполненныеРаботы Цикл
		...
		Для каждого ВидТО Из ВидыТО Цикл
			...
			Если СтрокаРеглРаботы[ВидТО + "Номер"] > 0 
				...
				И ТОВходитВИнтервал(ВидТО, СтрокаРеглРаботы[ВидТО + "ПоПлану"], СтрокаРеглРаботы[ВидТО + "Внеплановое"]) Тогда
...
&НаСервере
Процедура СкрытьЭлементыТЧВыпРабот()
	
	ВидыТО = ПолучитьВидыТО();
	
	Для Счетчик1 = 1 По Объект.ВыполненныеРаботы.Количество() Цикл
		Для Счетчик2 = 1 По ВидыТО.Количество() Цикл
			ИмяЭлементаСтрокиТЧ = ВидыТО[Счетчик2 - 1] + ИмяГруппыСтрокиТаблицы() + Строка(Счетчик1) + Строка(Счетчик2);
			ЭлементСтроки = ЭтотОбъект.Элементы.Найти(ИмяЭлементаСтрокиТЧ);
...
&НаСервере
Функция ТОВходитВИнтервал(ВидТО, ПоПлану, Внеплановое)
	
	Если Не ЗначениеЗаполнено(ПоПлану) Тогда
		// У внеплановых работ значение ПоПлану для следующего и предыдущего ТО не запонлено
		Возврат Ложь;
	КонецЕсли;
	...
	Если ИзмерительТО = Справочники.Автомобили.ИзмерительТО_ВРЕМЯ() Тогда
		...
		ТОНачалоИнтервала = НачалоИнтервалаТО(ВидТО, ПоПлану, Внеплановое);
	...
	ИначеЕсли ИзмерительТО = Справочники.Автомобили.ИзмерительТО_ПРОБЕГ() Тогда
		...
		МежсервисныйИнтервал = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Автомобиль, "МежсервисныйИнтервал");
		ТОНачалоИнтервала = НачалоИнтервалаТО(ВидТО, ПоПлану, Внеплановое);
		
		Если ВидТО = ИмяТекущееТО() Тогда
			ТОКонецИнтервала = МежсервисныйИнтервал * (Объект.ТекущийНомерТО + 1);
		ИначеЕсли ВидТО = ИмяСледующееТО() Тогда
			ТОКонецИнтервала = МежсервисныйИнтервал * (Объект.ТекущийНомерТО + 2);
		ИначеЕсли ВидТО = ИмяПрошлогоТО() Тогда
			ТОКонецИнтервала = МежсервисныйИнтервал * Объект.ТекущийНомерТО;
		Иначе
			ВызватьИсключение "Not implemented";
		КонецЕсли;
...
&НаСервере
Функция НачалоИнтервалаТО(ВидТО, ДатаАнализа, Внеплановое)
	...
	ИначеЕсли ИзмерительТО = Справочники.Автомобили.ИзмерительТО_ПРОБЕГ() Тогда
		...
		Если ВидТО = ИмяТекущееТО() Тогда
			Возврат МежсервисныйИнтервал * Объект.ТекущийНомерТО;
		ИначеЕсли ВидТО = ИмяСледующееТО() Тогда
			Возврат МежсервисныйИнтервал * (Объект.ТекущийНомерТО + 1);
		ИначеЕсли ВидТО = ИмяПрошлогоТО() Тогда
			Возврат МежсервисныйИнтервал  * (Объект.ТекущийНомерТО - 1);
		Иначе
			ВызватьИсключение "Not implemented";
...
Функция ТекущийНомерТО(знач Автомобиль, знач ДатаАнализа, знач Пробег)
	...
	Иначе
		Возврат ФормыСервисныеРаботыСервер.ТекущийНомерТОПрбег(Автомобиль, ДатаАнализа, Пробег);
	КонецЕсли;
	
КонецФункции

&НаСервереБезКонтекста
Функция ТаблицаПланТО(знач Автомобиль, ИзмерительТО, знач ДатаАнализа, знач Пробег)
	
	Если ИзмерительТО = Справочники.Автомобили.ИзмерительТО_ВРЕМЯ() Тогда
		Возврат Документы.РегистрацииТО.ТаблицаПланТО(ДатаАнализа, Автомобиль);
	Иначе
		Возврат Документы.РегистрацииТО.ТаблицаПланТОПробег(Пробег, Автомобиль);
	КонецЕсли;
КонецФункции
...
&НаСервереБезКонтекста
Функция ИзмерительТОВремя()
	Возврат Справочники.Автомобили.ИзмерительТО_ВРЕМЯ();
КонецФункции
