...
Процедура ОбновитьТумблерДиапазонаТО()
	...
	Для каждого ВидТО Из ВидыТО Цикл
		...
		Если ВидТО = ИмяПрошлогоВР() Тогда
			ЭлСписка.Представление = СтрШаблон("№%1 %2", ТекущийНомерТО - 1, Формат(ДатаПоИнтервалу, "ДФ=yyyy"));
		ИначеЕсли ВидТО = ИмяТекущееВР() Тогда
			ЭлСписка.Представление = СтрШаблон("№%1 %2", ТекущийНомерТО, Формат(ДатаПоИнтервалу, "ДФ=yyyy"));
